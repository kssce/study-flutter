// constraints 는 최소 너비, 최대 너비, 최소 높이, 최대 너비로 구성되며,
// 크기는 특정 너비와 높이로 구성됨
...
// 위젯은 자신의 size 를 오직 부모로부터 받은 contstraints 에 의해서만 결정할 수 있습니다.
...
// [ 렌더링 순서 ]
// 모든 위젯은 부모로부터 고유한 constraints 를 받는다. (최대 너비, 최소 너비 최디 높이, 최소 높이)
// 부모 위젯은 각 자식에게 자신의 제약조건이 무엇인지 알려주고, 각 자식에게 원하는 크기를 묻는다.
// 그 다음 자식을 x, y 축으로 하나씩 배치한다.
// 마지막으로 위젯은 부모에게 (제약조건 내에서 계산된) 자체 크기를 알려준다.
...
// UnconstrainedBox:
// 자신은 부모와 제약 조건과 똑같이 만들고
// 자식은 제약조건과 무관하게 레이아웃할 수 있다.
